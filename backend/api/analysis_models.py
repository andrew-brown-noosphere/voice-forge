"""
Analysis data models using Pydantic for request and response validation.
Renamed from Crawl to Analysis for better user experience.
"""
...
class AnalysisConfig(BaseModel):
    ...
    max_depth: int = Field(3, ge=1, le=10, description="Maximum depth to analyze")
    max_pages: Optional[int] = Field(None, ge=1, description="Maximum number of pages to analyze (unlimited if None)")
    ...
    user_agent: str = Field("VoiceForge-Analyzer/1.0 (+https://voiceforge.ai/bot)", description="User agent string (deprecated, use user_agent_mode)")

class AnalysisRequest(BaseModel):
    """Request to start a new analysis job."""
    domain: str = Field(..., description="Domain URL to analyze")
...
class AnalysisProgress(BaseModel):
    ...
    pages_analyzed: int = Field(0, description="Number of pages analyzed")
    pages_discovered: int = Field(0, description="Number of pages discovered")
    pages_failed: int = Field(0, description="Number of pages that failed to analyze")
    current_depth: int = Field(0, description="Current analysis depth")
    content_extracted: int = Field(0, description="Number of content pieces extracted")

class AnalysisStatus(BaseModel):
    """Status of an analysis job."""
    analysis_id: str = Field(..., description="Unique identifier for the analysis job")
    domain: str = Field(..., description="Domain being analyzed")
    state: AnalysisState = Field(..., description="Current state of the analysis")
    progress: AnalysisProgress = Field(default_factory=AnalysisProgress, description="Progress of the analysis")
    start_time: Optional[datetime] = Field(None, description="When the analysis started")
    end_time: Optional[datetime] = Field(None, description="When the analysis ended")
    error: Optional[str] = Field(None, description="Error message if the analysis failed")
    config: AnalysisConfig = Field(..., description="Configuration used for the analysis")
...
class ContentResponse(BaseModel):
    ...
    analysis_id: str = Field(..., description="ID of the analysis that found this content")
...
CrawlState = AnalysisState
CrawlConfig = AnalysisConfig
CrawlRequest = AnalysisRequest
CrawlProgress = AnalysisProgress
CrawlStatus = AnalysisStatus
