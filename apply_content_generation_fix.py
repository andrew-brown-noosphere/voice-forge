#!/usr/bin/env python3
"""
Apply the content generation fix to main.py
"""
...
def apply_fix():
    """Apply the content generation fix to the main API file."""
    
    main_api_path = '/Users/andrewbrown/Sites/noosphere/github/voice-forge/backend/api/main.py'
    ...
    # Create backup
    backup_path = main_api_path + '.backup_fix'
    with open(backup_path, 'w') as f:
        f.write(content)
    print(f"✅ Created backup: {backup_path}")
    
    # Define the new generate_content function
...
async def generate_content(
    ...
    # Find the pattern to replace - find the entire generate_content function
    pattern = r'@app\.post\("/rag/generate".*?(?=@app\.|if __name__|$)'
    ...
    print("✅ Content generation endpoint has been fixed!")
    print("\n🚀 NEXT STEPS:")
    print("1. Restart the backend server:")
    print("   cd backend && python -m uvicorn api.main:app --reload")
    print("2. Test content generation in the frontend")
    print("3. The endpoint will now ALWAYS return content, even if RAG fails")
